...
> **Revision 5 (flagship verticals unified):** `venues.html` rebuilt into the light panoramic system (was the last dark v1 page) with ALL original copy verbatim — hero, Revenue Recovery calculator, How-It-Works texts, compare grid, about, pilot form + guardrail — plus a new **Night Closeout Agent** console (the original voice-note line is the demo's input; register variance flagged, PO drafted, roster synced, partner brief sent, "Back Office After Close: Empty"). `legal.html` gains a hero (NLP pitch split H1/lede), an **Intake & Drafting Agent** console (inquiry → conflict check → engagement letter → docketed, "Attorney Minutes Spent: 0"), What-We-Deploy rows, and a CTA/sibling slab — the Stripe + Sheet Monkey DROP-IN block untouched. All six industries now have consoles on one design system.

> **Revision 4 (vertical demo pages):** Four industry pages added — `medical.html` (Chair-Fill Recovery console), `realestate.html` (Speed-to-Lead stopwatch pipeline), `ecommerce.html` (Support Triage queue on Shopify+Gorgias framing), `trades.html` (Dispatch Command with live SVG map). Each: NLP pitch as hero lede (verbatim), animated ops-console badged "Live Simulation // Sample Data", a What-We-Deploy fulfillment section naming the integratable stack (Vapi/Bland-class voice, Modento-class booking, Zillow/Slack, Shopify/Gorgias, Jobber/Housecall Pro), CTA slab routing to the homepage Calendly, and sibling-industry chips. Shared engine: `demos.js` (IntersectionObserver start, looping timelines, reduced-motion = static end state). Homepage industry links all now read "Explore the live build →"; palette + sitemap updated.
